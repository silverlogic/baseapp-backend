...
def test_migrate_legacy_reactable_fields_to_metadata_creates_metadata_rows():
    source_rows = [
        _ModelRowFactory(
            pk=1,
...
def test_migrate_legacy_reactable_fields_no_op_when_no_source_rows():
...
def test_reverse_migrate_legacy_reactable_fields_restores_source_fields():
    source_model = SimpleNamespace(
        _meta=SimpleNamespace(app_label="comments", model_name="comment"),
        objects=_SourceManager([]),
    )
...
def test_reverse_migrate_legacy_reactable_fields_no_op_when_content_type_missing():
    source_model = SimpleNamespace(
        _meta=SimpleNamespace(app_label="comments", model_name="comment"),
        objects=_SourceManager([]),
    )
