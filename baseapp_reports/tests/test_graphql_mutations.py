...
def test_user_can_get_report_types(django_user_client, graphql_user_client):
    ...
    assert len(content["data"]["allReportTypes"]["edges"]) == 11 # Created by baseapp_reports migrations
...
def test_user_can_get_report_type_filtered_by_content_type(django_user_client, graphql_user_client):
    ...
    response = graphql_user_client(REPORT_TYPES_GRAPHQL, variables={"targetObjectId": other_profile.relay_id})
    content = response.json()
    assert len(content["data"]["allReportTypes"]["edges"]) == 8 # 11 total, with 3 being exclusive to comments
