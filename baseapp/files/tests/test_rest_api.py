...
class TestFileCRUD:
    ...
    def test_retrieve_file(self, user_client, completed_file):
        ...
        assert data["id"] == completed_file.id
...
class TestSetParent:
    ...
    def test_set_parent_requires_ownership(self, client, standalone_file):
        ...
        response = client.post(
            ...
            {
                "parent_content_type": f"{user_ct.app_label}.{user_ct.model}",
                "parent_object_id": other_user.id,
            },
            format="json",
        )

        # Returns 404 because queryset filters by owner (file doesn't exist for this user)
        # This is correct REST API behavior - don't leak resource existence
