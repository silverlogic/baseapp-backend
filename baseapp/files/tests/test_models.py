...
def get_or_create_document_id(obj):
    """Helper to get or create a DocumentId for an object."""
...
class FileModelTest(TestCase):
    ...
    def test_comment_files_count(self):
        ...
        parent_doc_id = get_or_create_document_id(comment)

        File.objects.create(parent=parent_doc_id)
...
class FileTargetTest(TestCase):
    def test_file_target_creation(self):
        ...
        self.assertEqual(file_target.target.object_id, comment.pk)
    ...
    def test_file_target_str_method(self):
        ...
        expected_str = f"FileTarget for {file_target.target}"
        self.assertEqual(str(file_target), expected_str)

    def test_file_target_unique_constraint(self):
        comment = Comment.objects.create()
        doc_id = get_or_create_document_id(comment)

        FileTarget.objects.create(target=doc_id)

        file_target2, created = FileTarget.objects.get_or_create(target=doc_id)
    ...
    def test_can_attach_file_when_files_enabled(self):
        ...
        self.assertEqual(file_obj.parent.object_id, comment.pk)
...
class FileUtilsTest(TestCase):
    ...
    def test_set_files_parent_moves_file(self):
        ...
        parent_doc_id1 = get_or_create_document_id(comment1)

        file_obj = File.objects.create(
            parent=parent_doc_id1,
            file_content_type="image/png",
        )
        ...
        self.assertEqual(file_obj.parent.object_id, comment2.pk)
    ...
    def test_set_files_parent_same_parent(self):
        ...
        file_obj = File.objects.create(parent=parent_doc_id)
...
class FileTargetSignalsTest(TestCase):
    def test_file_creation_updates_file_target(self):
        ...
        file_target = FileTarget.objects.get(
            target__content_type=comment_content_type,
