...
@pytest.mark.asyncio
async def test_on_connect_sets_anonymous_user_when_header_missing():
...
@pytest.mark.asyncio
async def test_on_connect_sets_anonymous_user_when_key_not_found():
    consumer = _make_consumer()
...
@pytest.mark.asyncio
async def test_on_connect_sets_anonymous_user_when_key_is_expired():
...
@pytest.mark.asyncio
async def test_on_connect_sets_user_when_key_is_valid():
    api_key = await database_sync_to_async(f.APIKeyFactory)()
    unencrypted = await database_sync_to_async(APIKey.objects.decrypt)(
        encrypted_value=api_key.encrypted_api_key
    )
...
@pytest.mark.asyncio
async def test_on_connect_skips_auth_when_user_already_in_scope():
    sentinel = object()
    consumer = _make_consumer()
