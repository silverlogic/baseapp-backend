...
class TestAuthWithoutBaseappProfiles:
    @staticmethod
    def _detach_profile_if_present(user) -> None:
        if hasattr(user, "profile_id"):
            user.profile_id = None
            user.save(update_fields=["profile"])

    def test_user_retrieve_works_without_profiles(self, with_disabled_apps, user_client) -> None:
    ...
    def test_jwt_login_does_not_require_profiles(self, with_disabled_apps, client) -> None:
