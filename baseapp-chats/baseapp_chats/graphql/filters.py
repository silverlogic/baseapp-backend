...
class ChatRoomFilter(django_filters.FilterSet):
    ...
    def qs(self):
        qs = super().qs
        archived = self.data.get("archived", False)
        try:
            user_profile = self.request.user.current_profile
        except AttributeError:
            return qs.none()

        non_archived_rooms_subquery = ChatRoomParticipant.objects.filter(
            room=OuterRef("pk")
        ).filter(profile=user_profile, has_archived_room=archived)

        return qs.filter(
            participants__in=Subquery(non_archived_rooms_subquery.values("pk"))
        ).distinct()
    ...
    def filter_profile_id(self, queryset, name, value):
        ...
        return queryset.filter(participants__profile_id=pk).exclude()
