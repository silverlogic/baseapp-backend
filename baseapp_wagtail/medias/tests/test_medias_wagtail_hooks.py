...
class RichTextDocumentLinkHandlerTests(TestCase):
    ...
    def test_custom_document_link_handler(self) -> None:
        html = f'<a id="{self.document.id}" linktype="document">foo</a>'
        result = expand_db_html(html)
        self.assertIn('target="_blank"', result)

    def test_documents_with_full_url(self) -> None:
        html = f'<a id="{self.document.id}" linktype="document">foo</a>'
        result = expand_db_html(html)
...
class RichTextImageFormatsTests(TestCase):
    ...
    def test_full_width_image_format(self) -> None:
    ...
    def test_original_size_image_format(self) -> None:
